...
class User < ActiveRecord::Base
  ...
  def grouped_unread_notifications
    @grouped_unread_notifications ||= notification_query.bulk_counts[:grouped_unread_counts]
  end

  def unread_high_priority_notifications
    @unread_high_priority_notifications ||=
      notification_query.bulk_counts[:unread_high_priority_count]
  end

  def new_personal_messages_notifications_count
    @new_personal_messages_notifications_count ||=
      notification_query.bulk_counts[:new_personal_messages_count]
  end

  def unread_notifications
    @unread_notifications ||= notification_query.bulk_counts[:unread_low_priority_count]
  end

  def all_unread_notifications_count
    @all_unread_notifications_count ||= notification_query.bulk_counts[:unread_count]
  end
